...
test('Clase Ghost bien implementada', function(assert) {
  	...
  	setTimeout(function() {
		...
		assert.ok( typeof(ghost)!= "undefined" , "El phantom existe" );
		...
		ghost.move();
		assert.ok( ghost.x > 250 , "El phantom se mueve hacia la derecha cuando es posible" );
		...
		ghost.move();
		assert.ok( ghost.y > 240 , "El phantom se mueve hacia abajo cuando es posible" );
		...
		ghost.velX = 3;
		ghost.velY = 3;
		assert.ok( (ghost.x != 240 && ghost.y == 240) || (ghost.x == 240 && ghost.y != 240) , "El phantom elige una direccion al azar cuando encuentra una bifurcacion " );
