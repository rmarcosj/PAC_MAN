 ...
 // variables para contar frames/s, usadas por measureFPS
    ...
    var Ghost = function(id, ctx){
		...
		this.draw = function(){
			...
			// El cuerpo del phantom sólo debe dibujarse cuando el estado del mismo es distinto a Ghost.SPECTACLES
			...
			//
			// Test13 Tu código aquí
			// Si esl estado del phantom es Ghost.SPECTACLES
			// Mover el phantom lo más recto posible hacia la casilla de salida
			};
	...
	Pacman.prototype.move = function() {
		...
		for (var i = 0; i <numGhosts; i++) {
			if(thisLevel.checkIfHit(this.x,this.y,ghosts[i].x,ghosts[i].y,thisGame.TILE_WIDTH/2) && ghosts[i].state!=Ghost.VULNERABLE){
		...
		// test13 Tu código aquí.  Si chocamos contra un phantom y su estado es Ghost.VULNERABLE
		// cambiar velocidad del phantom y pasarlo a modo Ghost.SPECTACLES
...
test('Gafas', function(assert) {
	...
	setTimeout(function() {
		for (var i=0; i < 4; i++){
			game.ghosts[i].state = game.Ghost.SPECTACLES ; // ponemos el estado de los phantoms a modo SPECTACLES
	...
	// esperamos unos segundos. Se supone que los phantoms deben volver a casa y resucitar a modo NORMAL 
  	var done = assert.async();
  	setTimeout(function() {
		for (var i=0; i < 4; i++){
			assert.ok( game.ghosts[1].state == game.Ghost.NORMAL, "Los phantoms vuelven a su estado normal");
		}
		done();
